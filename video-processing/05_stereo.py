...
by 03_calibrate_to_reference.py) to use the empirical UTC mapping instead.
...
Usage (legacy, uses meta fps — minutes of misalignment):
    uv run 05_stereo_moon.py <left_video> <left_meta> <right_video> <right_meta>
...
def load_frame_to_utc(path: str, video_start: datetime, total_frames: int) -> np.ndarray:
    """
    Load an anchor table emitted by 03_calibrate_to_reference.py and build a
    ...
    Mirrors the loader in 04_rotate_moon.py:115-152.
    """
    with open(path) as f:
...
def main():
    ...
    parser.add_argument("--left-frame-to-utc", default=None,
                        help="Optional: anchor table from 03_calibrate_to_reference.py "
